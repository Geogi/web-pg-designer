...
export const databaseStart = (auto?: boolean) => async (dispatch: Function, getState: () => Root) => {
  ...
  try {
    ...
    const tableNames = (await pool.query(showTables)).rows.map((res: any) => res.table_name);
    const tables: Table[] = await Promise.all(tableNames.map(async (name: string) => {
      const primaryKeys = (await pool.query(showPrimaryKeys(name))).rows.map((res: any) => res.column_name);
      return ({
        name,
