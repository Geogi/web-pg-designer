...
export function createAction<T, P>(type: string, payloadFn: () => P): ThunkBuilder<P>;
export function createAction<T, P>(type: string, payloadFn: (arg: T) => P): FunctionBuilder<T, P>;
export function createAction<T, P>(type: string, payloadFn?: Function): Builder<T, P> {
  if (payloadFn === undefined) {
    return Object.assign(() => ({type}), {type, kind: "nude" as const});
  } else if (payloadFn.length === 0) {
    return Object.assign(() => ({type, payload: payloadFn()}), {type, kind: "thunk" as const});
  } else {
    return Object.assign((arg: T) => ({type, payload: payloadFn(arg)}), {type, kind: "function" as const});
  }
}
...
export function handle<S, P>(maker: NudeBuilder, handler: (s: S) => S): DefinedActionReducer<S>;
export function handle<S, P>(maker: ThunkBuilder<P>, handler: (s: S) => S): DefinedActionReducer<S>;
export function handle<S, P>(
  maker: FunctionBuilder<any, P>,
  handler: (s: S, p: P) => S):
  DefinedActionReducer<S>;
export function handle<S, P>(maker: Builder<S, P>, handler: Function): DefinedActionReducer<S> {
  return {
    type: maker.type,
    reducer: (s: S, p: any) => {
      if (maker.kind === "nude") {
